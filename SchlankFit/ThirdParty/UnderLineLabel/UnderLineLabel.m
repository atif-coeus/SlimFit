...
- (void) drawRect:(CGRect) rect
{
    if (shouldUnderline || shouldStrikeOut)
    {
        ...
        //calculate line height for some random simbol using its own font.[UIFont boldSystemFontOfSize:fontSize]
        int lineHeight = [@"a" sizeWithAttributes:@{NSFontAttributeName:self.font}].height;
        ...
        int topOffset = (self.frame.size.height - [mTotalTextString sizeWithAttributes:@{NSFontAttributeName:self.font}].height) / 2;
        ...
        for (int i = 0; i < [mTotalTextString length]; i++)
        {
            if ([mTotalTextString characterAtIndex:i] == '\n')
            {
                ...
                mPartOfTextStringWidth = [mPartOfTextString sizeWithAttributes:@{NSFontAttributeName:self.font}].width;
